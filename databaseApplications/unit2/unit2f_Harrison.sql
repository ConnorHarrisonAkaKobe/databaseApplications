...
--    of team_game_stats.
SELECT t.full_name, g.game_date, g.pts
...
-- ratings?
 -- that every movie has a rating and ratings are connected to the movies even when from a different table
...
-- Join: it takes the foreign key and conjoins it with another


-- INNER JOIN: keep only the rows that match on both sides


-- Table alias: nicknames for tables
