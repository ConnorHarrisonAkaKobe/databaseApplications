...
SELECT COUNT(*) AS player_count
FROM players p
LEFT JOIN player_season_stats s ON p.player_id = s.player_id AND season = '2025-26'
...
-- the first one has the stats and the second had the same thing but with the ones without values

-- In query 3 you filtered with IS NULL. Which table did that NULL
-- come from, and why is it NULL?
-- it came from player_season_stats and it is like that becaus it is unmatched

-- =====================================================================
-- VOCABULARY — your words, not the reference sheet's
-- =====================================================================

-- LEFT JOIN: use left side rows even if it is unmatched


-- Export: using a program to turn data into what you want as
